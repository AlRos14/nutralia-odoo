...
class ProductProduct(models.Model):
    ...
    def _compute_final_margin(self):
        for product in self:
            ...
            product.final_margin = (
                product.list_price_vat_excl - product.final_cost
            )
            if product.list_price_vat_excl == 0:
                product.final_margin_rate = 999.0
            else:
                product.final_margin_rate = (
                    (product.list_price_vat_excl - product.final_cost)
                    / product.list_price_vat_excl
                    * 100
                )
            if product.final_cost == 0:
                product.final_markup_rate = 999.0
            else:
                product.final_markup_rate = (
                    (product.list_price_vat_excl - product.final_cost)
                    / product.final_cost
                    * 100
                )